...
int* compareTriplets(int a_count, int* a, int b_count, int* b, int* result_count) {
    *result_count = 2;  // Set pointer value to size of array
    static int scores[2] = {0,0}; // Statically allocate array
    
    for(int i = 0; i < 3 ; i++){
        if (a[i] > b[i]){       // If Alice has more points
            scores[0] += 1;     // Increment Alice's score
        }
        else if (b[i] > a[i]){  // If Bob has more points
            scores[1] += 1;     // Increment Bob's score
        }
        // In case of equal points, do nothing
    }
    // Return array as integer pointer
    return scores;
}
